...
#define _TRACKBALL_ITEM(node)                                                                      \
    {.dev = NULL,                                                                                  \
     .polling_count = 0,                                                                           \
     .trigger = {.type = SENSOR_TRIG_DATA_READY, .chan = SENSOR_CHAN_ALL}},
...
K_MSGQ_DEFINE(zmk_trackballs_msgq, sizeof(zmk_trackballs_msg), CONFIG_ZMK_KSCAN_EVENT_QUEUE_SIZE,
              4);
...
static void zmk_trackballs_init_item(const char *node, uint8_t i, uint8_t abs_i) {
    ...
    int count = 0;
    do {
        err = sensor_trigger_set(trackballs[i].dev, &trackballs[i].trigger,
                                 zmk_trackballs_trigger_handler);
        if (err == -EBUSY) {
            count++;
            k_sleep(K_MSEC(20));
        }
...
#define _TRACKBALL_INIT(node)                                                                      \
    zmk_trackballs_init_item(DT_LABEL(node), local_index++, absolute_index++);
