...
enum pd_dir_mode { TIME_MODE, DISTANCE_MODE, EAGER_MODE };
...
static inline int pd_dir_time_mode(const struct behavior_point_device_directional_config *cfg,
                                   struct behavior_point_device_directional_data *data, int16_t dx,
                                   int16_t dy, int dt) {
    // 1. accumulate
    data->dx += dx;
    data->dy += dy;
...
static inline int pd_dir_distance_mode(const struct behavior_point_device_directional_config *cfg,
    ...
    case TWO_DIM: {
        if (abs(data->dx) >= cfg->step_size && abs(data->dx) >= abs(data->dy))
            return data->dx > 0 ? X_PLUS : X_MINUS;
        else if (abs(data->dy) >= cfg->step_size && abs(data->dy) >= abs(data->dx))
            return data->dy > 0 ? Y_PLUS : Y_MINUS;
    } break;
    default:
        LOG_ERR("unsupported flavor %d", cfg->flavor);
...
static inline int pd_dir_eager_mode(const struct behavior_point_device_directional_config *cfg,
    ...
    data->dt += dt;
    if (data->dx == 0 || (data->dx ^ dx) < 0 || data->dt > cfg->step_size) {
        data->dx = dx;
...
#define KP_INST(n)                                                                                 \
    ...
    DEVICE_DT_INST_DEFINE(n, behavior_point_device_directional_init, NULL,                         \
                          &behavior_pd_dir_data_##n, &behavior_pd_dir_config_##n, APPLICATION,     \
