...
static int motion_burst_read(const struct device *dev, uint8_t *buf, size_t burst_size) {
    ...
    uint8_t reg_buf[] = {PAW3395_REG_MOTION_BURST};
    const struct spi_buf tx_buf = {.buf = reg_buf, .len = ARRAY_SIZE(reg_buf)};
    const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};

    // todo: after sending the address, mosi should be held static until burst read is complete? how
    // to do it in zephyr
    err = spi_write_dt(&config->bus, &tx);
    if (err) {
...
static int upload_pwrup_settings(const struct device *dev) {
    ...
    int err;
    err = burst_write(dev, paw3395_pwrup_registers_addr1, paw3395_pwrup_registers_data1,
                      paw3395_pwrup_registers_length1);
    if (err) {
    ...
    err = burst_write(dev, paw3395_pwrup_registers_addr2, paw3395_pwrup_registers_data2,
                      paw3395_pwrup_registers_length2);
    if (err) {
...
static int set_run_mode(const struct device *dev, enum paw3395_run_mode run_mode) {
    ...
    err =
        burst_write(dev, paw3395_mode_registers_addr[mode_idx],
                    paw3395_mode_registers_data[mode_idx], paw3395_mode_registers_length[mode_idx]);
...
static int paw3395_attr_set(const struct device *dev, enum sensor_channel chan,
    ...
    case PAW3395_ATTR_RUN_DOWNSHIFT_TIME:
        err = set_downshift_time(dev, PAW3395_REG_RUN_DOWNSHIFT, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_REST1_DOWNSHIFT_TIME:
        err = set_downshift_time(dev, PAW3395_REG_REST1_DOWNSHIFT, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_REST2_DOWNSHIFT_TIME:
        err = set_downshift_time(dev, PAW3395_REG_REST2_DOWNSHIFT, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_REST1_SAMPLE_TIME:
        err = set_sample_time(dev, PAW3395_REG_REST1_PERIOD, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_REST2_SAMPLE_TIME:
        err = set_sample_time(dev, PAW3395_REG_REST2_PERIOD, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_REST3_SAMPLE_TIME:
        err = set_sample_time(dev, PAW3395_REG_REST3_PERIOD, PAW3395_SVALUE_TO_TIME(*val));
        break;

    case PAW3395_ATTR_RUN_MODE:
        err = set_run_mode(dev, PAW3395_SVALUE_TO_RUNMODE(*val));
        break;
...
static int paw3395_async_init_configure(const struct device *dev) {
    ...
    if (!err) {
        err = set_sample_time(dev, PAW3395_REG_REST1_PERIOD, CONFIG_PAW3395_REST1_SAMPLE_TIME_MS);
    }

    if (!err) {
        err = set_sample_time(dev, PAW3395_REG_REST2_PERIOD, CONFIG_PAW3395_REST2_SAMPLE_TIME_MS);
    }
    if (!err) {
        err = set_sample_time(dev, PAW3395_REG_REST3_PERIOD, CONFIG_PAW3395_REST3_SAMPLE_TIME_MS);
    }

    // downshift time for each rest mode
    if (!err) {
        err = set_downshift_time(dev, PAW3395_REG_RUN_DOWNSHIFT,
                                 CONFIG_PAW3395_RUN_DOWNSHIFT_TIME_MS);
    }

    if (!err) {
        err = set_downshift_time(dev, PAW3395_REG_REST1_DOWNSHIFT,
                                 CONFIG_PAW3395_REST1_DOWNSHIFT_TIME_MS);
    }

    if (!err) {
        err = set_downshift_time(dev, PAW3395_REG_REST2_DOWNSHIFT,
...
static int paw3395_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
                               sensor_trigger_handler_t handler) {
    LOG_INF("trigger_set");
...
#define PAW3395_DEFINE(n)                                                                          \
    ...
    DEVICE_DT_INST_DEFINE(n, paw3395_init, NULL, &data##n, &config##n, POST_KERNEL,                \
                          CONFIG_SENSOR_INIT_PRIORITY, &paw3395_driver_api);
